...
public class GodTokens {
	...
	public static void startSkill(final String player, final GodToken token) {
		...
		else if(!(skill.equalsIgnoreCase("Dodge") || skill.equalsIgnoreCase("Explosive"))) {
			p.getInventory().addItem(ItemStackGenerator.createItem(Material.GOLD_SWORD, 0, 0,
					ChatColor.RED + token.getName(), Arrays.asList(ChatColor.GRAY + "Right-Click to use this!"), true));
		}
	}
